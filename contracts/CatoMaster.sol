...
// CatoMaster is the master of Cato. He can make Cato and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once CATO is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract CatoMaster is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. CATOs to distribute per block.
        uint256 lastRewardBlock; // Last block number that CATOs distribution occurs.
        uint256 accCatoPerShare; // Accumulated CATOs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus CATO period ends.
    uint256 public bonusEndBlock;
    // Block number when mint CATO period ends.
    uint256 public mintEndBlock;
    // CATO tokens created per block.
    uint256 public catoPerBlock;
    // Bonus muliplier for 5~20 days cato makers.
    uint256 public constant BONUSONE_MULTIPLIER = 20;
    // Bonus muliplier for 20~35 cato makers.
    ...
    // The block number when CATO mining starts.
    ...
    constructor(
        CatoToken _cato,
        address _devaddr,
        uint256 _catoPerBlock,
        uint256 _startBlock
    ) public {
        cato = _cato;
        devaddr = _devaddr;
        catoPerBlock = _catoPerBlock;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        require(lpTokenPID[address(_lpToken)] == 0, "CatoMaster:duplicate add.");
    ...
    // Update the given pool's CATO allocation point. Can only be called by the owner.
    ...
    // Handover the catotoken mintage right.
    function handoverCatoMintage(address newOwner) public onlyOwner {
        cato.transferOwnership(newOwner);
    }
    ...
    // View function to see pending CATOs on frontend.
    function pendingCato(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCatoPerShare = pool.accCatoPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 catoReward = multiplier.mul(catoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCatoPerShare = accCatoPerShare.add(catoReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accCatoPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 catoReward = multiplier.mul(catoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        cato.mint(devaddr, catoReward.div(15));
        cato.mint(address(this), catoReward);
        pool.accCatoPerShare = pool.accCatoPerShare.add(catoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCatoPerShare).div(1e12).sub(user.rewardDebt);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accCatoPerShare).div(1e12);
        if (pending > 0) safeCatoTransfer(msg.sender, pending);
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from CatoMaster.
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accCatoPerShare).div(1e12);
        safeCatoTransfer(msg.sender, pending);
    ...
    function safeCatoTransfer(address _to, uint256 _amount) internal {
