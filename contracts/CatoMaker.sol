...
contract CatoMaker is Ownable {
    ...
    constructor(ICatPinFactory _factory, address _bar, address _cato, address _weth) public {
        require(address(_factory) != address(0) && _bar != address(0) && 
            _cato != address(0) && _weth != address(0), "invalid address");
        factory = _factory;
        cato = _cato;
        bar = _bar;
        weth = _weth;
    ...
    function convert(address token0, address token1) public {
        ...
        IERC20(weth).transfer(factory.getPair(weth, cato), wethAmountToBar);
        _toCATO(wethAmountToBar, bar);
        IERC20(weth).transfer(factory.getPair(weth, cato), wethAmountToBurn);
        _toCATO(wethAmountToBurn, address(1));
    }
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        ICatPinPair pair = ICatPinPair(factory.getPair(token, weth));
    ...
    function _toCATO(uint256 amountIn, address to) internal {
        ICatPinPair pair = ICatPinPair(factory.getPair(weth, cato));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
