...
contract CatnipMaker is Ownable {
    ...
    constructor(ICatnipFactory _factory, address _bar, address _catnip, address _weth) public {
        require(address(_factory) != address(0) && _bar != address(0) && 
            _catnip != address(0) && _weth != address(0), "invalid address");
        factory = _factory;
        catnip = _catnip;
        bar = _bar;
        weth = _weth;
    ...
    function convert(address token0, address token1) public {
        ...
        IERC20(weth).transfer(factory.getPair(weth, catnip), wethAmountToBar);
        _toCATNIP(wethAmountToBar, bar);
        IERC20(weth).transfer(factory.getPair(weth, catnip), wethAmountToBurn);
        _toCATNIP(wethAmountToBurn, address(1));
    }
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        ICatnipPair pair = ICatnipPair(factory.getPair(token, weth));
    ...
    function _toCATNIP(uint256 amountIn, address to) internal {
        ICatnipPair pair = ICatnipPair(factory.getPair(weth, catnip));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
