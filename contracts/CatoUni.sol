...
contract CatoUni is Ownable {
    ...
    // Block number of distributing CATO period ends.
    uint256 public endBlock;
    // Block number of distributing bonus CATO period ends.
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256 _cato, uint256 _uni) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        _cato = user.amount.mul(accCatoPerShare).div(1e12).sub(user.catoRewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pendingCato = user.amount.mul(pool.accCatoPerShare).div(1e12).sub(user.catoRewardDebt);
        uint256 pendingUni = user.amount.mul(pool.accUniPerShare).div(1e12).sub(user.uniRewardDebt);
        user.amount = user.amount.add(_amount);
        user.catoRewardDebt = user.amount.mul(pool.accCatoPerShare).div(1e12);
