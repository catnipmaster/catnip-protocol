...
import "./CatPinERC20.sol";
import "./libraries/Math.sol";
import "./libraries/UQ112x112.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/ICatPinFactory.sol";
import "./interfaces/ICatPinCallee.sol";
import "./interfaces/ICatPinPair.sol";
import "./CatPinSlippageToken.sol";
...
contract CatPinPair is CatPinERC20 {
    ...
    CatPinSlippageToken public stoken;
    ...
    modifier lock() {
        require(unlocked == 1, "CatPin: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "CatPin: TRANSFER_FAILED");
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "CatPin: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
        stoken = new CatPinSlippageToken(0);
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), "CatPin: OVERFLOW");
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ICatPinFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            address migrator = ICatPinFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                require(liquidity > 0 && liquidity != uint256(-1), "CatPin: Bad desired liquidity");
            } else {
                require(migrator == address(0), "CatPin: Must not have migrator");
        ...
        require(liquidity > 0, "CatPin: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        {
            ...
            require(amount0 > 0 && amount1 > 0, "CatPin: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, "CatPin: INSUFFICIENT_OUTPUT_AMOUNT");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "CatPin: INSUFFICIENT_LIQUIDITY");
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, "CatPin: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0) ICatPinCallee(to).CatPinCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint256 amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint256 amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, "CatPin: INSUFFICIENT_INPUT_AMOUNT");
        {
    ...
    function dealSlippageWithIn(address[] calldata path, uint256 amountIn, address to, bool ifmint) external lock returns (uint256 amountOut) {
        require(path.length == 2, "CatPin: INVALID_PATH");
        address _token0 = token0;
        ...
        // avoids stack too deep errors
        {
            ...
            require(amount >= amountIn, "CatPin: Invalid Amount");
        }
