...
contract CatnipVoterProxy {
    ...
    //sum user deposit catnipnum
    function balanceOf(address _voter) external view returns (uint256) {
        uint256 _votes = 0;
        uint256 _vCtLpTotal;
        uint256 _vUserLp;
        uint256 _vCtCatnipNum;
        uint256 _vUserCatnipNum;
        uint256 _vTmpPoolId;
        IERC20 _vLpToken;
        for (uint256 i = votePoolMap.iterateStart(); votePoolMap.iterateValid(i); i = votePoolMap.iterateNext(i)) {
            //user deposit catnipnum = user_lptoken*contract_catnipnum/contract_lptokens
            (, _vTmpPoolId) = votePoolMap.iterateGet(i);
            if (chef.poolLength() > _vTmpPoolId) {
                (_vLpToken, , , ) = chef.poolInfo(_vTmpPoolId);
                _vCtLpTotal = ICatnipPair(address(_vLpToken)).totalSupply();
                if (_vCtLpTotal == 0) {
                    continue;
                }
                (_vUserLp, ) = chef.userInfo(_vTmpPoolId, _voter);
                _vCtCatnipNum = votes.balanceOf(address(_vLpToken));
                _vUserCatnipNum = _vUserLp.mul(_vCtCatnipNum).div(_vCtLpTotal);
                _votes = _votes.add(_vUserCatnipNum);
            }
        }
