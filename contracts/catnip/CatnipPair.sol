...
import "./CatnipERC20.sol";
import "./libraries/Math.sol";
import "./libraries/UQ112x112.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/ICatnipFactory.sol";
import "./interfaces/ICatnipCallee.sol";
import "./interfaces/ICatnipPair.sol";
import "./CatnipSlippageToken.sol";
...
contract CatnipPair is CatnipERC20 {
    ...
    CatnipSlippageToken public stoken;
    ...
    modifier lock() {
        require(unlocked == 1, "Catnip: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Catnip: TRANSFER_FAILED");
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Catnip: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
        stoken = new CatnipSlippageToken(0);
    }
    ...
    function _update(
        ...
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), "Catnip: OVERFLOW");
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ICatnipFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            address migrator = ICatnipFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                require(liquidity > 0 && liquidity != uint256(-1), "Catnip: Bad desired liquidity");
            } else {
                require(migrator == address(0), "Catnip: Must not have migrator");
        ...
        require(liquidity > 0, "Catnip: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        {
            ...
            require(amount0 > 0 && amount1 > 0, "Catnip: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(
        ...
        require(amount0Out > 0 || amount1Out > 0, "Catnip: INSUFFICIENT_OUTPUT_AMOUNT");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "Catnip: INSUFFICIENT_LIQUIDITY");
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, "Catnip: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0) ICatnipCallee(to).CatnipCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint256 amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint256 amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, "Catnip: INSUFFICIENT_INPUT_AMOUNT");
        {
    ...
    function dealSlippageWithIn(
        ...
        require(path.length == 2, "Catnip: INVALID_PATH");
        address _token0 = token0;
        ...
        // avoids stack too deep errors
        {
            ...
            require(amount >= amountIn, "Catnip: Invalid Amount");
        }
