...
contract CatoVoterProxy {
    ...
    //sum user deposit catonum
    function balanceOf(address _voter) external view returns (uint256) {
        uint256 _votes = 0;
        uint256 _vCtLpTotal;
        uint256 _vUserLp;
        uint256 _vCtCatoNum;
        uint256 _vUserCatoNum;
        uint256 _vTmpPoolId;
        IERC20 _vLpToken;
        for (uint256 i = votePoolMap.iterateStart(); votePoolMap.iterateValid(i); i = votePoolMap.iterateNext(i)) {
            //user deposit catonum = user_lptoken*contract_catonum/contract_lptokens
            (, _vTmpPoolId) = votePoolMap.iterateGet(i);
            if (chef.poolLength() > _vTmpPoolId) {
                (_vLpToken, , , ) = chef.poolInfo(_vTmpPoolId);
                _vCtLpTotal = ICatPinPair(address(_vLpToken)).totalSupply();
                if (_vCtLpTotal == 0) {
                    continue;
                }
                (_vUserLp, ) = chef.userInfo(_vTmpPoolId, _voter);
                _vCtCatoNum = votes.balanceOf(address(_vLpToken));
                _vUserCatoNum = _vUserLp.mul(_vCtCatoNum).div(_vCtLpTotal);
                _votes = _votes.add(_vUserCatoNum);
            }
        }
